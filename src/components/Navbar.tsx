...
const Navbar:React.FC = () => {
  ...
  return (
    ...
    <div className=" flex justify-between items-center px-3 lg:px-52 ">
       ...
       <div className={`absolute  lg:static py-36 lg:py-0 justify-start items-center lg:justify-between lg:items-start top-0 left-0 h-[100vh] md:h-full w-full lg:w-1/3 flex flex-col md:flex-row bg-[#0d1114] transition-all duration-500  delay-100 ease-in-out z-10 lg:bg-transparent gap-10 lg:opacity-100  font-medium text-2xl lg:text-base  text-[#F78D26] ${show? "opacity-100" : "opacity-0"}`}>
        
            <span onClick={()=>{setshow(!show)}} className="absolute top-8 right-5 p-1 text-4xl lg:hidden"><FontAwesomeIcon icon={faClose} /></span>
